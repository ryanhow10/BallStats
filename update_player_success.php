...
<body>
    ...
    $sql = "UPDATE player SET team_id = ? WHERE player_id = ?";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param('ii', $teamId, $player_id); //Binding values into query
    $stmt->execute();
    echo ('<center>');
    echo('<h3>Update success! Player has moved teams.</h3>');
    echo ('</center>');
